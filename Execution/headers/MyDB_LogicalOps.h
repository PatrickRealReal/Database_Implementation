...
public: 
	...
	//    Note that each attribute in this output spec should have a 1-1 correspondence with the expressions
	//    in exprsToCompute
	...
	//
    LogicalAggregate (LogicalOpPtr inputOp, MyDB_TablePtr outputSpec, MyDB_TablePtr final_output_Spec, vector <ExprTreePtr> &exprsToCompute,
                      vector <ExprTreePtr> &groupings, bool isSingleTable, string single_table_alias, vector<string> &regular_select_projection) : inputOp (inputOp), outputSpec (outputSpec), exprsToCompute (exprsToCompute),
                        groupings (groupings), single_table_alias(single_table_alias), is_single_table(isSingleTable), regular_select_projection(regular_select_projection), final_output_Spec(final_output_Spec) {}
...
private:
    ...
    vector<string> regular_select_projection;  // 由于agg输出的顺序是先 group再aggregate，结果会跟valueToSelect的顺序出现不一致
};
...
public:
	...
	LogicalTableScan (MyDB_TableReaderWriterPtr inputSpec, MyDB_TablePtr outputSpec, MyDB_StatsPtr inputStats, 
		...
		inputStats (inputStats), selectionPred (selectionPred), exprsToCompute (exprsToCompute), alias(alias) {}
